...
wp core install --path=/var/www/ --url=$DOMAIN --title=$DOMAIN --admin_user=$ADMIN_USER --admin_password=$ADMIN_PASS --admin_email=$ADMIN_EMAIL --allow-root

wp user create --path=/var/www/ $USER_NAME $USER_EMAIL --user_pass=$USER_PASS --role=author --allow-root
